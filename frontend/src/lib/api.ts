...
export const api = {
  companies: {
    ...
    search: (q: string) => req<Pick<Company, 'id'|'name'|'industry'|'city'|'country'|'slug'>[]>(`/companies/search?q=${encodeURIComponent(q)}`),
    trending: () => req<Company[]>('/companies/trending'),
  ...
  connections: {
    send: (companyId: string, role: string, message?: string) =>
      req<Connection>('/connections', { method: 'POST', body: JSON.stringify({ companyId, role, message }) }),
    list: () => req<Connection[]>('/connections'),
    respond: (id: string, status: 'ACCEPTED' | 'DECLINED') =>
      req<Connection>(`/connections/${id}`, { method: 'PATCH', body: JSON.stringify({ status }) }),
  },
  bookmarks: {
    list: () => req<Company[]>('/bookmarks'),
    add: (companyId: string) => req<void>(`/bookmarks/${companyId}`, { method: 'POST' }),
    remove: (companyId: string) => req<void>(`/bookmarks/${companyId}`, { method: 'DELETE' }),
  },
};
